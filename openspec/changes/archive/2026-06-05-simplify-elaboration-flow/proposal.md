...
- **`chorus_pm_validate_elaboration` marks the whole elaboration complete (requires `idea:admin`).** It is an Idea-level action: takes only `ideaUuid`, sets Idea → `elaborated` / `elaborationStatus` → `resolved`, and does not target or mutate any single round. Precondition: the Idea has ≥1 round and every round is answered. It does not tag per-question issues or create follow-up rounds — opening a follow-up round is just another `chorus_pm_start_elaboration` call. The tool description and every skill that calls it MUST state that **human confirmation is required before calling it (except in YOLO mode)**.
