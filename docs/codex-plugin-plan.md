...
- **插件内容**(`plugins/chorus/`): 7 个 skills + 3 个 reviewer sub-agents(proposal / task / code）+ hooks(`SessionStart` + 3× `PostToolUse`)。
...
│       ├── hooks.json                          ← SessionStart + 3× PostToolUse
│       ├── hooks/
...
│       │   └── on-post-verify-task.sh          ← 末任务验收后:archive(A)+ code-review gateway(C)+ completion report(B)提醒
│       ├── skills/
...
│       │   │   ├── SKILL.md
│       │   │   └── agents/openai.yaml
│       │   └── chorus-code-reviewer/           ← 末端 code-review gateway:审 Idea 聚合变更,verdict 落在 idea
│       │       └── SKILL.md
│       └── README.md
│
└── public/
...
5. 启用 `[features] hooks = true`;Chorus hooks 随 Codex 插件打包,由 Codex 从 plugin manifest / bundled `hooks.json` 自动加载

脚本完全幂等,重跑会原地替换 API key、MCP URL,不产生重复 TOML 段。若检测到旧版 installer 复制到 `~/.codex/hooks.json` 的 Chorus entries,脚本会展示命中行并询问是否清理,避免和插件自带 hooks 重复触发。
...
| Hook 启用 | `grep '^hooks = true' ~/.codex/config.toml` + Codex TUI `/hooks` | `hooks = true`,且 Chorus bundled hooks 已 review/trust |
| Skill 可用 | 输入 `$chorus:chorus` | 弹出 skill 卡片 |

### 3.5 变更 / 卸载

- **换 API key 或 URL**: 重跑 `install-codex.sh`(全自动覆盖)。
- **卸载**: Codex TUI `/plugins` → Uninstall;然后手动从 `~/.codex/config.toml` 删除 `[mcp_servers.chorus]`、`[mcp_servers.chorus.http_headers]`、`[plugins."chorus@chorus-plugins"]` 和 `[marketplaces.chorus-plugins]` 块。若机器上还残留旧版 copied hooks,从 `~/.codex/hooks.json` 删除包含 `hooks/chorus/run-hook.sh` 的 entries。
...
- **`[features] hooks = true`**: `hooks` 是当前 canonical feature key;`codex_hooks` 只是 deprecated alias。Installer 会移除旧的 `hooks` / `codex_hooks` key 并写入 `hooks = true`,避免重复 TOML key。
...
| `PostToolUse` | `.*chorus_admin_verify_task` | `on-post-verify-task.sh` | 末任务 verify 后提醒:OpenSpec archive(A)+ code-review gateway(C,spawn chorus-code-reviewer 审 Idea 聚合变更)+ completion report(B);三者按 A→C→B 顺序拼接 |
...
- `systemMessage` → **给用户看** — TUI 用 `warning:` 前缀渲染(`warning:` 是 Codex 硬编码的 prefix,不代表这是警告)。Routine success 不应使用它。
- `additionalContext` → **给 agent 看,也会给用户看** — Codex 会把它记录为 developer/context message,同时 TUI 也会原样 echo(`hook context:` 前缀)。当前没有 hidden-only hook injection 通道。

`hooks/hook-output.sh` 封装了 jq/fallback 两条路径生成这个 JSON。

### 6.3 Plugin-bundled `hooks.json`
...
```json
{
  "hooks": {
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "${PLUGIN_ROOT}/hooks/on-session-start.sh",
            "timeout": 20,
    ...
    "PostToolUse": [
      { "matcher": ".*chorus_pm_submit_proposal", "hooks": [ { "type": "command", "command": "${PLUGIN_ROOT}/hooks/on-post-submit-proposal.sh", "timeout": 10 } ] },
      { "matcher": ".*chorus_submit_for_verify", "hooks": [ { "type": "command", "command": "${PLUGIN_ROOT}/hooks/on-post-submit-for-verify.sh", "timeout": 10 } ] },
      { "matcher": ".*chorus_admin_verify_task", "hooks": [ { "type": "command", "command": "${PLUGIN_ROOT}/hooks/on-post-verify-task.sh", "timeout": 10 } ] }
    ]
  }
}
```

- **插件自带**: hooks 由 `plugins/chorus/hooks.json` 随插件分发,不再复制到用户级 `~/.codex/hooks.json`。
- **路径解析**: bundled hooks 使用 `${PLUGIN_ROOT}` 指向当前启用的插件根目录,避免 installer 写死 plugin cache 版本路径。
- **信任流程**: plugin-bundled hooks 仍是 non-managed hooks,用户需要在 Codex TUI `/hooks` review/trust。Installer 只负责启用 `[features] hooks = true`。
- **legacy 清理**: 旧版 installer 写过的 `~/.codex/hooks.json` entries 会包含 `hooks/chorus/run-hook.sh`;新版 installer 检测到后展示命中行并询问是否自动清理。
